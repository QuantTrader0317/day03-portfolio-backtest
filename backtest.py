#!/usr/bin/env python3
"""
Multi-asset equal-weight portfolio backtest
- Data: yfinance (adjusted close), daily returns
- Rebalance: monthly or quarterly (EOM/EoQ)
- Costs: per-day return deduction from turnover (bps)
- Cash: subtract annual rf as a daily drag
...
Usage (examples):
  python backtest.py
  python backtest.py --tickers SPY,QQQ,TLT,GLD --start 2005-01-01 --rf 0.02 --reb M --tc_bps 5 \
                     --target_vol 0.10 --leverage_cap 2.0

Notes:
- Keep parameter choices conservative to avoid overfitting.
- This is a learning scaffold, not investment advice.
"""
...
class Config:
    ...
    rf_annual: float = 0.02           # cash rate, e.g., 0.02 = 2% per year
    rebalance: str = "M"              # 'M' monthly, 'Q' quarterly
    tc_bps: int = 5                   # transaction cost per rebalance change (bps)
...
def apply_vol_targeting(r: pd.DataFrame, w_t: pd.DataFrame, target_vol: float, cap: float) -> pd.DataFrame:
    ...
    lookback = 60  # days; simple and conservative
    # rms of individual vols as a conservative proxy for portfolio vol
    ...
    # apply scale only on rebalance rows (where weights are set)
...
def parse_args() -> Config:
    ...
    p.add_argument("--reb", type=str, default="M", choices=["M", "Q"], help="Rebalance freq: M or Q")
